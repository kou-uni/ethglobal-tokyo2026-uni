...
produces the same 33 / 11 / 7 that `npm run seed -- 1` produces in process** — and a test
...
npm run seed -- 1  # one night, run through the real router
...
**`npm run seed -- 1` is the night used in the pitch** — 51 arrive, 33 settle, 11 are dropped,
