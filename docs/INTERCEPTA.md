...
これは採用を検討するための調査・提案文書。**APIの疎通や統合は未実施で、以下のフローやデモは実装済みではない。** 公式情報と私たちの考察を区別する。プロダクト定義は[CONCEPT.md](CONCEPT.md)を参照。
...
これは[CONCEPT.mdのdeny例](CONCEPT.md#deny--12-of-52)にある、支払い元のリスクによる停止を実際の外部判定につなぐ提案である。単にダッシュボードへ危険度を表示するだけではない。
