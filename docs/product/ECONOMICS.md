...
> **判定ごとの台帳と、任意の手数料署名の検証・保持まで実装しました。回収は未実装です。**
> 設定と検証は [ROUTING-FEES.md](../build/ROUTING-FEES.md)。以下のbatch回収・キャッシュバックは将来設計です。
...
判定1件の仕事を記録し、本人への支払いとは別に、薄い手数料の認可を任意で受け取ります。
本人への支払いから差し引きません。金額は設定値で、ガス代を実測した値ではありません。

```mermaid
flowchart LR
  A["🤖 エージェント"] -->|"依頼"| Y["⬜ Yohaku<br/>捌く"]
  A -->|"本人へ直接"| W["🧑 本人の財布"]
  A -->|"別の任意認可"| F["⚙️ 手数料台帳<br/><small>回収は未実装</small>"]
...
**現在は台帳と、別のexact認可を暗号学的に検証して保持する部分まで実装しています。**
未署名・署名済み・期限内の署名済み額を分け、換金・broadcastはゼロと明記します。
`batch-settlement` は対応一覧を確認した段階で、コミットや回収は実行していません。
共通仕様だけではEVM側の具体的な署名・回収方式を確定できないためです。
...
| **流量が出てから** | **判定件数によるトラフィック課金** | 捌いた件数に比例する。**使われるほど払う**が、**捌けた分しか払わない** |
...
| 「手数料署名を別に保持し、売り手へのpayToは変えません」 | 「一括回収済みです」 — **batch回収は未実装** |

---

*実装されているのは決済そのものだけです（Base Sepolia で2件、チェーンで確認）。
手数料の台帳・任意署名の保持は実装済み。**手数料回収とキャッシュバックは未実装です。***
