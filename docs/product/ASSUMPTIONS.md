...
**`npm run seed -- 1` is the night quoted in the pitch.** It was chosen as *the run closest
...
| settled | 2,612 JPYC | 1,484–4,377 |
...
| B2 | auto ~33 / deny ~11 / **2 reach her** | **`npm run seed -- 1`**, chosen as closest to the mean by the same rule as before — when the categories changed, the rule re-picked the seed. Reproducible | 🟡 |
