...
**反映・検証の更新：** PR #28と#29はmainへ反映済み。Studio公開版のHTML・JSは承認済み版と一致し、
スマホ引き継ぎ後にPCへ2件の回答が到着。合計0.001121 test USDCを独立したチェーン照合で確認した。
記録は [experience-public-payment.json](evidence/experience-public-payment.json)。
以下は再現用の手順。新ホストへ移す場合はそのoriginで改めて確認し、この記録を新環境の完了報告に転用しない。
